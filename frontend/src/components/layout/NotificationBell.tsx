...
export const NotificationBell: React.FC = () => {
  ...
  return (
    <div className="relative" ref={bellRef}>
      ...
      >
        <svg 
          className={`w-5 h-5 ${isAnimating ? 'animate-bounce text-cyan-500' : ''}`} 
          fill="none" 
          stroke="currentColor" 
          viewBox="0 0 24 24"
        >
        ...
        {badgeText && (
          <span className={`absolute top-0 right-0 flex items-center justify-center min-w-[1.25rem] h-5 px-1 text-[10px] font-bold text-white bg-teal-500 rounded-full border-2 border-white transition-transform ${isAnimating ? 'scale-110' : 'scale-100'}`}>
            {badgeText}
          </span>
        )}
      </button>
